...
async def test_install_conan_packages_basic(mock_run_command, client_session: ClientSession):
...
@pytest.mark.anyio
@patch('main.run_command')
async def test_install_conan_packages_with_remote(mock_run_command, client_session: ClientSession):
    ...
    call_args = mock_run_command.call_args[0][0]
    expected_cmd = [
        "conan", "install", "/path/to/project/conanfile.py", 
        "--remote", "conancenter", "--format=json"
    ]
    assert call_args == expected_cmd
...
async def test_install_conan_packages_with_settings_and_options(
    ...
    call_args = mock_run_command.call_args[0][0]
    expected_cmd = [
        "conan", "install", "/home/user/project/conanfile.py",
        "-s:h", "os=Linux", "-s:h", "arch=armv8", "-s:h", "compiler=gcc", 
        "-s:h", "compiler.version=11",
        "-o:h", "fPIC=True", "-o:h", "shared=False",
        "--build=missing",
        "--format=json"
    ]
    assert call_args == expected_cmd
